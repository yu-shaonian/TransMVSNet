...
class PatchEmbedding(nn.Module):
    def __init__(self, channel, patch_size1, patch_size2, dim, height =1200 , width=800):
        super().__init__()
        ...
        num_patches_max = (height * width *channel) // (patch_size1 * patch_size2 *4)
        patch_dim = 4 * patch_size1 * patch_size2
    ...
    def forward(self, x):
        B, C, W, H = x.shape
        x = rearrange(x, 'b (c d1) (w p1) (h p2) -> b (w h c) (d1 p1 p2)', d1=4, p1=self.patch_size1,
                      p2=self.patch_size2,)  # 1, 3, 224, 224 = 1 ,3 ,(14, 16),(14, 16) => 1, 196, 768
        x1 = self.to_patch_embedding(x)

        num_patches = (H * W * C) // (self.patch_size1 * self.patch_size2 * 4)
...
class Mvsformer(nn.Module):
    def __init__(self,channel=48, patch_size1=16,patch_size2=16, dim=512,num_heads=8):
        ...
        self.conv1 = nn.Sequential(
                ...
                Conv2d(channel// 3, channel, 3, 1, padding=1))
        patch_dim = 4 * patch_size1 * patch_size2
    ...
    def forward(self, x):
        ...
        # decoder
        B, P ,E = x4.shape
        x = self.embedding_to_patch(x4)

        cls_token = x[:, -1, :]
        x_patch = x[:, :(P -1), :]

        x = x_patch + cls_token.unsqueeze(1)
        # [B, P-1,E]
        # 1, 3, 224, 224 = 1 ,3 ,(14, 16),(14, 16) => 1, 196, 768
        x = x.reshape(B,C,W,H)
        # x = self.conv1(x)
...
class MvsformerDecoder(nn.Module):
    def __init__(self, channel=192*3, patch_size1=16,patch_size2=16, dim=100):
        super().__init__()
        # batch, channel, image_width, image_height = x.shape
        # assert image_width % patch_size == 0 and image_height % patch_size == 0, "Image size must be divided by the path size!"
        self.patch_size1 = patch_size1
...
if __name__ == '__main__':
    ...
    input6 = torch.rand(1,48,256,320).to(device)
